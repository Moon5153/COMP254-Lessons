...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
   ...
   * private Position<Entry<K,V>> treeSearch(Position<Entry<K,V>> p, K key) {
   ...
   Position<Entry<K, V>> curr = p; // set curr position to p
  
   // loop that continues until key is found or end of a path is reached
   while(!isExternal(curr))
   {
       // compare key and current key
       int comp = compare(key, curr.getElement());
       if (comp == 0) // key found; exit the loop
           break;
